...
2. **設定您的內容傳遞網路**：使用這些詳細資料，在您的內容傳遞網路提供者的控制台中建立記錄轉送工作 (或手動上傳記錄)。 若為CloudFront，您只能使用&#x200B;**AWS CLI**&#x200B;來使用主控台或完成傳遞設定；請參閱[CloudFront (AWS CLI)](/help/overview/log-forwarding/cloudfront-cli.md)。
