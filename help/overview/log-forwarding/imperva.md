...
4. 輸入下列內容：
   ...
   | **路徑** | 將要儲存記錄檔案的資料夾之位置。使用 `<Amazon S3 bucket name>/<log folder>` 格式。例如：`MyBucket/MyImpervaLogFolder`。 | `Amazon S3 bucket name` 是來自 LLM Optimizer 設定頁面的&#x200B;**貯體名稱**。![貯體名稱](/help/overview/assets/log-forwarding/imperva/imperva-bucket-name.png) 記錄資料夾是來自 LLM Optimizer 設定頁面的&#x200B;**路徑**。![路徑](/help/overview/assets/log-forwarding/imperva/imperva-path.png) |
