---
user-guide-title: LLM Optimizer
breadcrumb-title: LLM Optimizer
user-guide-description: 利用 Adobe LLM Optimizer 提升您的品牌。 追蹤提及次數、揭露洞察，並主導 AI 驅動的搜尋。 掌控您的能見度，馬上開始最佳化！
source-git-commit: d7e723161836027dcdde931378f5d0f776a1ecfc
workflow-type: tm+mt
source-wordcount: '171'
ht-degree: 95%

---


# LLM Optimizer {#using}

+ [LLM Optimizer](/help/home.md)
+ LLM Optimizer Essentials{#essentials}
  + [LLM Optimizer 概觀](/help/overview/overview.md)
  + [快速入門](/help/overview/quick-start.md)
  + [最佳做法](/help/tutorials/best-practices.md)
  + [針對類別、主題、提示和要追蹤的其他品牌的最佳做法](/help/overview/best-practices-topics-prompts.md)
+ 儀表板{#dashboards}
  + [儀表板概觀](/help/dashboards/dashboards-overview.md)
  + [品牌存在感](/help/dashboards/brand-presence.md)
  + [代理式流量](/help/dashboards/agentic-traffic.md)
  + [轉介流量](/help/dashboards/referral-traffic.md)
  + [URL 檢查器](/help/dashboards/url-inspector.md)
  + 機會{#opportunities}
    + [機會概觀](/help/dashboards/opportunities-overview.md)
    + [代理式流量錯誤](/help/dashboards/opportunities/agentic-traffic-errors.md)
    + [遭 robots.txt 封鎖的流量](/help/dashboards/opportunities/traffic-blocked-by-robots.md)
    + [復原內容能見度](/help/dashboards/opportunities/recover-content-visibility.md)
    + [新增適合 LLM 讀取的摘要](/help/dashboards/opportunities/add-llm-friendly-summaries.md)
    + [新增相關常見問題集](/help/dashboards/opportunities/add-relevant-faqs.md)
    + [新增多媒體逐字稿摘要](/help/dashboards/opportunities/add-multimedia-transcript-summaries.md)
    + [新增目錄](/help/dashboards/opportunities/add-table-of-contents.md)
    + [簡化複雜的內容](/help/dashboards/opportunities/simplify-complex-content.md)
    + [維基百科分析](/help/dashboards/opportunities/wikipedia-analysis.md)
    + [YouTube 情緒分析](/help/dashboards/opportunities/youtube-sentiment-analysis.md)
    + [Reddit 情緒分析](/help/dashboards/opportunities/reddit-sentiment-analysis.md)
    + [引用情緒分析](/help/dashboards/opportunities/cited-sentiment-analysis.md)
    + [擴充產品目錄](/help/dashboards/opportunities/enrich-product-catalog.md)
    + [擴充產品詳細資訊頁面](/help/dashboards/opportunities/enrich-product-detail-pages.md)
  + [客戶設定](/help/dashboards/customer-configuration.md)
  + [協作](/help/dashboards/collaboration.md)
+ BYOCDN 記錄轉送{#log-forwarding}
  + [概觀](/help/overview/log-forwarding/log-forwarding-overview.md)
  + [Akamai](/help/overview/log-forwarding/akamai.md)
  + [Cloudflare](/help/overview/log-forwarding/cloudflare.md)
  + [CloudFront](/help/overview/log-forwarding/cloudfront.md)
  + [CloudFront (AWS CLI)](/help/overview/log-forwarding/cloudfront-cli.md)
  + [Fastly](/help/overview/log-forwarding/fastly.md)
  + [Imperva](/help/overview/log-forwarding/imperva.md)
  + [其他 (手動上傳)](/help/overview/log-forwarding/other.md)
+ 其他資源{#resources}
  + 邊緣最佳化{#optimize-at-edge}
    + [邊緣最佳化概觀](/help/dashboards/optimize-at-edge/overview.md)
    + [檢索 API 金鑰](/help/dashboards/optimize-at-edge/retrieve-api-keys.md)
    + [AEM Cloud Service 管理的內容傳遞網路 (Fastly)](/help/dashboards/optimize-at-edge/aemcs-managed-cdn.md)
    + [Fastly (BYOCDN)](/help/dashboards/optimize-at-edge/fastly-byocdn.md)
    + [Akamai (BYOCDN)](/help/dashboards/optimize-at-edge/akamai-byocdn.md)
    + [Cloudflare (BYOCDN)](/help/dashboards/optimize-at-edge/cloudflare-byocdn.md)
    + [CloudFront (BYOCDN)](/help/dashboards/optimize-at-edge/cloudfront-byocdn.md)
    + [Azure正門(BYOCDN)](/help/dashboards/optimize-at-edge/azure-front-door-byocdn.md)
    + [Apache HTTP 伺服器](/help/dashboards/optimize-at-edge/apache-http-server.md)
  + [存取控制](/help/dashboards/access-control.md)
  + [Adobe Analytics 整合](/help/dashboards/adobe-analytics-integration.md)
  + [Google Analytics整合](/help/dashboards/google-analytics-integration.md)
