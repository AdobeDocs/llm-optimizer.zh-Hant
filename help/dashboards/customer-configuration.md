...
如果您使用以品牌為中心的體驗，請導覽至&#x200B;**品牌管理**，以設定及設定品牌、品牌別名，並定義要追蹤的競爭對手。 **品牌管理**&#x200B;也可用來設定與品牌相關URL的整合，例如Google Search Console、Adobe Analytics和CDN記錄轉送。 您可以按一下對應的標籤來執行此操作：GSC、CDN等。
...
對於使用以品牌為中心的體驗的客戶，若要新增主題和提示，請瀏覽至&#x200B;**提示管理**。
...
新增查詢後，該查詢會顯示在「客戶設定」儀表板的[提示](#prompts-brand)索引標籤中（傳統體驗）或&#x200B;**提示管理** （品牌導向體驗）。 取自 Google Search Console 的提示會在「**來源**」欄中標示 Google Search Console 圖示。 該圖示可協助您區別以真實使用者搜尋行為作為基礎的提示、手動新增的提示，或是來自其他來源的提示。
